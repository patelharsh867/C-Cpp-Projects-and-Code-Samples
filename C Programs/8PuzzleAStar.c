...
int heur(int block[]);
void prepend(struct elementstruct*, struct elementstruct*, int operator1);
int goal(int* block);
int notonqueue(int block[]);
struct elementstruct* bestnodefromqueue();
...
struct elementstruct* newnode = newelement();

  while (1)
    {
    struct elementstruct* node = bestnodefromqueue();
      if (node == NULL) {
	printf("done!\n");
...
void prepend(struct elementstruct* newnode, struct elementstruct* oldnode, int op)
{
      newnode->next = top;
      top = newnode;
...
struct elementstruct* bestnodefromqueue()
{
      ...
      int totalpathcost;
      struct elementstruct* to_return = NULL;
...
struct elementstruct* newelement()
{
    struct elementstruct* t = (struct elementstruct*)malloc(sizeof *t);
      if (t==NULL) return NULL;
      t->valid = 1;
      t->str =  (char *)malloc(sizeof(char)*(strlen(t)+1));
